    ...
    with session.get(base_url + page) as response:
        ...
        if data['success']:
            ...
            for auction in data['auctions']:
                ...
                if "Crimson" in auction['item_name']:
                    # if the auction isn't a) claimed and is b) BIN
                    if not auction['claimed'] and auction['bin'] == True:
...
def main():
    ...
    if len(results):  # if there's results to print
        ...
        for result in results:
            print("Auction UUID: " + str(result[0][0]) + " | Item Name: " + str(
                result[0][1]) + " | Item price: {:,}".format(result[0][2]),
                  " | Second lowest BIN: {:,}".format(result[1]) + " | Time to refresh AH: " + str(round(done, 2)))
